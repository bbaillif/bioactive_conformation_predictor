...
class KinasesCMGCSplit(ProteinSplit):
    
    def __init__(self, 
                 split_type: str = 'kinases_cmgc', 
                 split_i: int = 0) -> None:
    ...
    def split_dataset(self):
        ...
        pdbbind_table = pdbbind_table.merge(chembl_target_df, 
                                            left_on='Uniprot ID', 
                                            right_on='accession')
        pdbbind_table = pdbbind_table[pdbbind_table['protein_class_desc'].str.contains('protein kinase')]
        
        l_class = ['enzyme  kinase  protein kinase  cmgc']
