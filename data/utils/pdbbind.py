...
from data import PDBLigandExpo
...
class PDBbindMetadataProcessor() :
    ...
    def __init__(self, 
                 root: str = '/home/bb596/hdd/PDBbind/',
        ...
        self.pdb_ligand_expo = PDBLigandExpo()
    ...
    def get_master_dataframe(self, 
                             corrections: bool = True) -> pd.DataFrame():
    ...
    def get_ligands(self, 
        ...
        for pdb_id in tqdm(pdb_ids) :
            ...
            if mol is not None : # cleaning might fail
    ...
    def get_ligand(self,
                   pdb_id: str,
                   clean: bool = True) -> Union[Mol, None]:
        ...
        try :
            ...
            if mol is None :
                raise Exception('Reading mol2 gives None')
            ...
            try :
                ...
                if mol is None :
                    raise Exception('Reading sdf gives None')
            except Exception as e :
        ...
        if mol is not None :
            if clean :
                mol = self.clean_ligand(mol, pdb_id)
        return mol
    ...
    def clean_ligand(self,
        ...
        try :
            ...
            if ideal_mol is None :
                raise Exception('RDKit failed to parse PDB ideal ligand SMILES')
            ...
            if len(match) != standard_ideal.GetNumHeavyAtoms() :
                ...
                match = standard_ideal.GetSubstructMatch(standard_mol)
                if not len(match) == standard_ideal.GetNumAtoms() :
                    raise Exception('No match found between actual and ideal ligand')
            
        except Exception as e:
            print(f'Cleaning failed for {pdb_id}')
            print(str(e))
        else :
